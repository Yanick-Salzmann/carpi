...
/*    class SimpleFileIO : public cdm::FileIO {
    ...
    public:
        ...
        void Open(const char *file_name, uint32_t file_name_size) override {
            ...
            name_buffer.back() = '\0';
            _file = open(name_buffer.data(), O_CREAT | O_RDWR, S_IRUSR | S_IWUSR);
            if(_file < 0) {
    ...
    cdm::Time WidevineHost::GetCurrentWallTime() {
        ...
        return static_cast<cdm::Time>(tv.tv_sec);
    }
    ...
    cdm::FileIO *WidevineHost::CreateFileIO(cdm::FileIOClient *) {
        return nullptr; //new SimpleFileIO{client};
    }
